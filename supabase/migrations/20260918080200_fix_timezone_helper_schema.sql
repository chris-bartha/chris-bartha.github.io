-- Fix: get_quiz_public_metrics is security invoker, and anon has no USAGE on
-- schema private, so calling private.player_timezone() from inside it failed with
-- "permission denied for schema private" and the dashboard showed its error card
-- to every visitor. Granting execute on the function was not enough; the schema
-- itself is closed, deliberately, and widening it to make one helper reachable
-- would be the wrong trade.
--
-- The helper moves to public instead, as security definer. It returns a single
-- timezone string and reads nothing a caller could not already infer, so there is
-- nothing to leak; the private schema stays shut.

create or replace function public.quiz_player_timezone()
returns text
language sql
stable
security definer
...
revoke all on function public.quiz_player_timezone() from public, anon, authenticated;
grant execute on function public.quiz_player_timezone() to anon, authenticated, service_role;

comment on function public.quiz_player_timezone() is
...
drop function if exists private.player_timezone();
...
begin
  ...
  where timezone.name = coalesce(
    nullif(public.quiz_player_timezone(), ''),

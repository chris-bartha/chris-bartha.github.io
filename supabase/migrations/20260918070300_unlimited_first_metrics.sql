-- /metrics, rebuilt around the only mode that still produces rounds.
--
-- Standard ten-question rounds are gone everywhere but the Fifth Grader ladder,
-- so the numbers the dashboard led with -- average percentage, best percentage,
-- perfect scores, the score distribution -- stop moving. They are not deleted:
-- 1,353 standard rounds happened and they still say something. They move to an
-- archive, and the run becomes the headline. Only 61 of her 1,405 rounds were
-- ever standard -- 95.7% were already Unlimited before the mode was removed --
-- so this is the dashboard finally describing what she actually does.
--
-- Everything the old payload carried is still carried, under the same keys, so
-- a browser holding a ten-minute-old metrics.js keeps working while the new one
-- propagates. Streaks and daily activity have always counted every attempt
-- regardless of mode and are untouched by this.

-- quiz_questions is readable by signed-in players only, and the dashboard is
-- public. pool_size and question_bank come from inside a security-invoker
-- function, so an anonymous visitor would silently see zeros. One narrow view,
-- counts only, no prompts or answers.
create or replace view public.quiz_pool_sizes
with (security_invoker = false)
as
select
  category.id as category_id,
  count(question.id) filter (where question.is_active)::integer as pool_size
from public.quiz_categories as category
left join public.quiz_questions as question on question.category_id = category.id
where category.is_active
group by category.id;

revoke all on table public.quiz_pool_sizes from public, anon, authenticated;
grant select on table public.quiz_pool_sizes to anon, authenticated, service_role;

comment on view public.quiz_pool_sizes is
  'How many active questions each category holds. Counts only -- deliberately public for the metrics page.';
...
comment on function public.get_quiz_public_metrics(text) is
  'Unlimited-first metrics for /metrics, with the pre-change standard rounds kept under "archive".';
